...
function RASI() { 
	...
	var savDate = new Date(locDate.getTime());
	if (document.getElementById('pp_ayan') == null) return;
	let localtime = document.getElementById('pp_local');
	let b_local = false;
	let str_time = 'UTC/GMT';
	...
	var classes, ids, value, d, date2, date_local;
	var showWheel1 = 0, showWheel2 = 0, showMoon = 0, showTable = 0, showPanchang = 0;
...
$const.date = d;
	...
	if (b_local) {
		var ddate2 = (savDate.getDate());
		var dmonth2 = (savDate.getMonth()+1);
		var dyear2 = (savDate.getFullYear());
		var dhour2 = (savDate.getHours());
		var dminute2 = (savDate.getMinutes());
		var dseconds2 = (savDate.getSeconds());
		date_local = {
			day: ddate2,
			month: dmonth2,
			year: dyear2,
		...
		$moshier.julian.calc (date_local);
		date_local.universalDate = $moshier.julian.toGregorian ({
			julian: date_local.julian
		});
		date_local.universalDateString =
		dyear2 + ' ' +
		dmonth2 + ' ' +
		ddate2 + ' ' +
		dhour2 + ':' +
...
//	$moshier.kepler.calc (d, $moshier.body.earth);
	moon_s = moon_s > 9.999 ? moon_s.toFixed(1) : '0'+moon_s.toFixed(1);
	sun_s = sun_s > 9.999 ? sun_s.toFixed(1) : '0'+sun_s.toFixed(1);
	mercury_s = mercury_s > 9.999 ? mercury_s.toFixed(1) : '0'+mercury_s.toFixed(1);
	venus_s = venus_s > 9.999 ? venus_s.toFixed(1) : '0'+venus_s.toFixed(1);
	mars_s = mars_s > 9.999 ? mars_s.toFixed(1) : '0'+mars_s.toFixed(1);
	jupiter_s = jupiter_s > 9.999 ? jupiter_s.toFixed(1) : '0'+jupiter_s.toFixed(1);
	saturn_s = saturn_s > 9.999 ? saturn_s.toFixed(1) : '0'+saturn_s.toFixed(1);
	uranus_s = uranus_s > 9.999 ? uranus_s.toFixed(1) : '0'+uranus_s.toFixed(1);
	neptune_s = neptune_s > 9.999 ? neptune_s.toFixed(1) : '0'+neptune_s.toFixed(1);
	pluto_s = pluto_s > 9.999 ? pluto_s.toFixed(1) : '0'+pluto_s.toFixed(1);
	nnode_s = nnode_s > 9.999 ? nnode_s.toFixed(1) : '0'+nnode_s.toFixed(1);
	asc_s = asc_s > 9.999 ? asc_s.toFixed(1) : '0'+asc_s.toFixed(1);
...
var iter = 0;
	...
	remd.universalDateString =
		...
		if (b_local)
			tzo = tz*60/86400;
		od.universalDate = $moshier.julian.toGregorian ({julian: (od.julian+tzo) });
...
//	d = remd;
	...
	$moshier.planet.calc ($moshier.body.saturn);
	if ((saturn - $moshier.body.saturn.position.apparentLongitude) < 0.0 ) { saturnR = '<sub>R</sub>'; }
	$moshier.planet.calc ($moshier.body.jupiter);
	if ((jupiter - $moshier.body.jupiter.position.apparentLongitude) < 0.0 ) { jupiterR = '<sub>R</sub>'; }
	$moshier.planet.calc ($moshier.body.mars);
	if ((mars - $moshier.body.mars.position.apparentLongitude) < 0.0 ) { marsR = '<sub>R</sub>';  }
	
	$moshier.planet.calc ($moshier.body.venus);
	if ((venus - $moshier.body.venus.position.apparentLongitude) < 0.0 ) { venusR = '<sub>R</sub>'; }
	$moshier.planet.calc ($moshier.body.mercury);
	if ((mercury - $moshier.body.mercury.position.apparentLongitude) < 0.0 ) { mercuryR = '<sub>R</sub>'; }
	$moshier.planet.calc ($moshier.body.neptune);
	if ((neptune - $moshier.body.neptune.position.apparentLongitude) < 0.0 ) { neptuneR = '<sub>R</sub>'; }
	$moshier.planet.calc ($moshier.body.uranus);
	if ((uranus - $moshier.body.uranus.position.apparentLongitude) < 0.0 ) { uranusR = '<sub>R</sub>'; }
	$moshier.planet.calc ($moshier.body.pluto);
	if ((pluto - $moshier.body.pluto.position.apparentLongitude) < 0.0 ) { plutoR = '<sub>R</sub>'; }
...
var	sun2_l,moon2_l,mercury2_l,venus2_l,mars2_l,jupiter2_l,saturn2_l,uranus2_l,neptune2_l,pluto2_l;
	...
	if (!b_local)
		tab = "<h4 >" + d.universalDateString+" "+str_time+"<br/>";
	else
		tab = "<h4 >" + date_local.universalDateString+" "+str_time+"<br/>";
	...
	tab = tab + "<tr><td style='font-family:monospace,mono!important;'>Sun&#9;<span style='font-family:Saravali!important;'>a</span></td><td><p style=\"color:"+zods[Math.floor(sun/30.0)][1]+"\">"+zods[Math.floor(sun/30.0)][0]+"</p>&#09;"+("00"+Math.floor(sun - (Math.floor(sun/30)*30.0))).substr(-2) + "°"+ ("00"+Math.floor(sun_m)).substr(-2) + "'" + (sun_s) + "\"</td><td><p style=\"color:"+zods[Math.floor(sun2/30.0)][1]+"\">"+zods[Math.floor(sun2/30.0)][0]+"</p>&#09;"+("00"+Math.floor(sun2 - (Math.floor(sun2/30)*30.0))).substr(-2) + "°"+ ("00"+Math.floor(sun2_m)).substr(-2) + "'" + (sun2_s) + "\"</td><td style='font-family:monospace,mono!important;'>" + nax[sun2_n] + "</td></tr>" ;  
	tab = tab + "<tr><td style='font-family:monospace,mono!important;'>Moon&#9;<span style='font-family:Saravali!important;'>b</span></td><td><p style=\"color:"+zods[Math.floor(moon/30.0)][1]+"\">"+zods[Math.floor(moon/30.0)][0]+"</p>&#09;"+("00"+Math.floor(moon - (Math.floor(moon/30)*30.0))).substr(-2) + "°"+ ("00"+Math.floor(moon_m)).substr(-2) + "'" + (moon_s)+ "\"</td><td><p style=\"color:"+zods[Math.floor(moon2/30.0)][1]+"\">"+zods[Math.floor(moon2/30.0)][0]+"</p>&#09;"+("00"+Math.floor(moon2 - (Math.floor(moon2/30)*30.0))).substr(-2) + "°"+ ("00"+Math.floor(moon2_m)).substr(-2) + "'" + (moon2_s) + "\"</td><td style='font-family:monospace,mono!important;'>"+ nax[moon2_n] + "</td></tr>"; 
	tab = tab + "<tr><td style='font-family:monospace,mono!important;'>Mercury&#9;<span style='font-family:Saravali!important;'>c</span></td><td><p style=\"color:"+zods[Math.floor(mercury/30.0)][1]+"\">"+zods[Math.floor(mercury/30.0)][0]+"</p>&#09;"+("00"+Math.floor(mercury - (Math.floor(mercury/30)*30.0))).substr(-2) + "°"+ ("00"+Math.floor(mercury_m)).substr(-2) + "'"+(mercury_s) +"\"" + mercuryR+"</td><td><p style=\"color:"+zods[Math.floor(mercury2/30.0)][1]+"\">"+zods[Math.floor(mercury2/30.0)][0]+"</p>&#09;"+("00"+Math.floor(mercury2 - (Math.floor(mercury2/30)*30.0))).substr(-2) + "°"+ ("00"+Math.floor(mercury2_m)).substr(-2) + "'"+(mercury2_s) +"\"" + mercuryR+"</td><td style='font-family:monospace,mono!important;'>" + nax[mercury2_n] + "</td></tr>" ;  
	tab = tab + "<tr><td style='font-family:monospace,mono!important;'>Venus&#9;<span style='font-family:Saravali!important;'>d</span></td><td><p style=\"color:"+zods[Math.floor(venus/30.0)][1]+"\">"+zods[Math.floor(venus/30.0)][0]+"</p>&#09;"+("00"+Math.floor(venus - (Math.floor(venus/30)*30.0))).substr(-2) + "°"+ ("00"+Math.floor(venus_m)).substr(-2) + "'"+ (venus_s) + "\""+venusR+"</td><td><p style=\"color:"+zods[Math.floor(venus2/30.0)][1]+"\">"+zods[Math.floor(venus2/30.0)][0]+"</p>&#09;"+("00"+Math.floor(venus2 - (Math.floor(venus2/30)*30.0))).substr(-2) + "°"+ ("00"+Math.floor(venus2_m)).substr(-2) + "'"+ (venus2_s) + "\""+venusR+"</td><td style='font-family:monospace,mono!important;'>" + nax[venus2_n] + "</td></tr>" ;  
	tab = tab + "<tr><td style='font-family:monospace,mono!important;'>Mars&#9;<span style='font-family:Saravali!important;'>e</span></td><td><p style=\"color:"+zods[Math.floor(mars/30.0)][1]+"\">"+zods[Math.floor(mars/30.0)][0]+"</p>&#09;"+("00"+Math.floor(mars - (Math.floor(mars/30)*30.0))).substr(-2) + "°"+ ("00"+Math.floor(mars_m)).substr(-2) + "'"+ (mars_s) + "\""+marsR+"</td><td><p style=\"color:"+zods[Math.floor(mars2/30.0)][1]+"\">"+zods[Math.floor(mars2/30.0)][0]+"</p>&#09;"+("00"+Math.floor(mars2 - (Math.floor(mars2/30)*30.0))).substr(-2) + "°"+ ("00"+Math.floor(mars2_m)).substr(-2) + "'"+ (mars2_s) + "\""+marsR+"</td><td style='font-family:monospace,mono!important;'>" + nax[mars2_n] +"</td></tr>" ;  
	tab = tab + "<tr><td style='font-family:monospace,mono!important;'>Jupiter&#9;<span style='font-family:Saravali!important;'>f</span></td><td><p style=\"color:"+zods[Math.floor(jupiter/30.0)][1]+"\">"+zods[Math.floor(jupiter/30.0)][0]+"</p>&#09;"+("00"+Math.floor(jupiter - (Math.floor(jupiter/30)*30.0))).substr(-2) + "°"+ ("00"+Math.floor(jupiter_m)).substr(-2) + "'"+ (jupiter_s) + "\""+ jupiterR+"</td><td><p style=\"color:"+zods[Math.floor(jupiter2/30.0)][1]+"\">"+zods[Math.floor(jupiter2/30.0)][0]+"</p>&#09;"+("00"+Math.floor(jupiter2 - (Math.floor(jupiter2/30)*30.0))).substr(-2) + "°"+ ("00"+Math.floor(jupiter2_m)).substr(-2) + "'"+ (jupiter2_s) + "\""+ jupiterR+"</td><td style='font-family:monospace,mono!important;'>" + nax[jupiter2_n] +"</td></tr>" ;  
	tab = tab + "<tr><td style='font-family:monospace,mono!important;'>Saturn&#9;<span style='font-family:Saravali!important;'>g</span></td><td><p style=\"color:"+zods[Math.floor(saturn/30.0)][1]+"\">"+zods[Math.floor(saturn/30.0)][0]+"</p>&#09;"+("00"+Math.floor(saturn - (Math.floor(saturn/30)*30.0))).substr(-2) + "°"+ ("00"+Math.floor(saturn_m)).substr(-2) + "'"+ (saturn_s) + "\""+ saturnR+"</td><td><p style=\"color:"+zods[Math.floor(saturn2/30.0)][1]+"\">"+zods[Math.floor(saturn2/30.0)][0]+"</p>&#09;"+("00"+Math.floor(saturn2 - (Math.floor(saturn2/30)*30.0))).substr(-2) + "°"+ ("00"+Math.floor(saturn2_m)).substr(-2) + "'"+ (saturn2_s) + "\""+ saturnR+"</td><td style='font-family:monospace,mono!important;'>" + nax[saturn2_n] + "</td></tr>" ;  
	tab = tab + "<tr><td style='font-family:monospace,mono!important;'>Uranus&#9;<span style='font-family:Saravali!important;'>h</span></td><td><p style=\"color:"+zods[Math.floor(uranus/30.0)][1]+"\">"+zods[Math.floor(uranus/30.0)][0]+"</p>&#09;"+("00"+Math.floor(uranus - (Math.floor(uranus/30)*30.0))).substr(-2) + "°"+ ("00"+Math.floor(uranus_m)).substr(-2) + "'"+ (uranus_s) + "\""+ uranusR+"</td><td><p style=\"color:"+zods[Math.floor(uranus2/30.0)][1]+"\">"+zods[Math.floor(uranus2/30.0)][0]+"</p>&#09;"+("00"+Math.floor(uranus2 - (Math.floor(uranus2/30)*30.0))).substr(-2) + "°"+ ("00"+Math.floor(uranus2_m)).substr(-2) + "'"+ (uranus2_s) + "\""+ uranusR+"</td><td style='font-family:monospace,mono!important;'>"+ nax[uranus2_n] + "</td></tr>" ;  
	tab = tab + "<tr><td style='font-family:monospace,mono!important;'>Neptune&#9;<span style='font-family:Saravali!important;'>i</span></td><td><p style=\"color:"+zods[Math.floor(neptune/30.0)][1]+"\">"+zods[Math.floor(neptune/30.0)][0]+"</p>&#09;"+("00"+Math.floor(neptune - (Math.floor(neptune/30)*30.0))).substr(-2) + "°"+ ("00"+Math.floor(neptune_m)).substr(-2) + "'"+ (neptune_s) + "\""+ neptuneR+"</td><td><p style=\"color:"+zods[Math.floor(neptune2/30.0)][1]+"\">"+zods[Math.floor(neptune2/30.0)][0]+"</p>&#09;"+("00"+Math.floor(neptune2 - (Math.floor(neptune2/30)*30.0))).substr(-2) + "°"+ ("00"+Math.floor(neptune2_m)).substr(-2) + "'"+ (neptune2_s) + "\""+ neptuneR+"</td><td style='font-family:monospace,mono!important;'>"+nax[neptune2_n]+"</td></tr>" ;  
	tab = tab + "<tr><td style='font-family:monospace,mono!important;'>Pluto&#9;<span style='font-family:Saravali!important;'>j</span></td><td><p style=\"color:"+zods[Math.floor(pluto/30.0)][1]+"\">"+zods[Math.floor(pluto/30.0)][0]+"</p>&#09;"+("00"+Math.floor(pluto - (Math.floor(pluto/30)*30.0))).substr(-2) + "°"+ ("00"+Math.floor(pluto_m)).substr(-2) + "'"+ (pluto_s) + "\""+ plutoR+"</td><td><p style=\"color:"+zods[Math.floor(pluto2/30.0)][1]+"\">"+zods[Math.floor(pluto2/30.0)][0]+"</p>&#09;"+("00"+Math.floor(pluto2 - (Math.floor(pluto2/30)*30.0))).substr(-2) + "°"+ ("00"+Math.floor(pluto2_m)).substr(-2) + "'"+ (pluto2_s) + "\""+ plutoR+"</td><td style='font-family:monospace,mono!important;'>"+nax[pluto2_n]+"</td></tr>" ;  
	tab = tab + "<tr><td style='font-family:monospace,mono!important;'>N. Node&#9;<span style='font-family:Saravali!important;'>T</span></td><td><p style=\"color:"+zods[Math.floor(nnode/30.0)][1]+"\">"+zods[Math.floor(nnode/30.0)][0]+"</p>&#09;"+("00"+Math.floor(nnode - (Math.floor(nnode/30)*30.0))).substr(-2) + "°"+ ("00"+Math.floor(nnode_m)).substr(-2) + "'" + (nnode_s) + "\"<sub>R</sub></td><td><p style=\"color:"+zods[Math.floor(nnode2/30.0)][1]+"\">"+zods[Math.floor(nnode2/30.0)][0]+"</p>&#09;"+("00"+Math.floor(nnode2 - (Math.floor(nnode2/30)*30.0))).substr(-2) + "°"+ ("00"+Math.floor(nnode2_m)).substr(-2) + "'" + (nnode2_s) + "\"<sub>R</sub></td><td style='font-family:monospace,mono!important;'>"+nax[nnode2_n]+ "</td></tr>" ;  
	tab = tab + "<tr><td style='font-family:monospace,mono!important;'>Asc.&#9;A</td><td><span style=\"color:"+zods[Math.floor(asc2/30.0)][1]+"\">"+zods[Math.floor(asc2/30.0)][0]+"</span>&#09;"+("00"+Math.floor(asc2 - (Math.floor(asc2/30)*30.0))).substr(-2) + "°"+ ("00"+Math.floor(asc_m)).substr(-2) + "'" + (asc_s) + "\"</td><td><span style=\"color:"+zods[Math.floor(asc22/30.0)][1]+"\">"+zods[Math.floor(asc22/30.0)][0]+"</span>&#09;"+("00"+Math.floor(asc22 - (Math.floor(asc22/30)*30.0))).substr(-2) + "°"+ ("00"+Math.floor(asc2_m)).substr(-2) + "'" + (asc2_s) + "\"</td><td style='font-family:monospace,mono!important;'>"+ nax[asc2_n]+"</td></tr></table>";
	}
...
var	p_name = [ 
	...
	tab = tab + "<p class='ppos'>Time: <br />" + d.universalDateString+" "+str_time+"</p>";
	...
	tab = tab + "<p class='ppos'>Time: <br />" + d.universalDateString+ " "+str_time+"</p>";
	tab = tab + "<div class='zodiac' style='transform: rotate("+((asc2+270)%360)+"deg);-webkit-transform: rotate("+((asc2+270)%360)+"deg);'>";
//	$JQ(".zodiac").css({"transform" : "rotate(" + ((asc2+270)%360) + "deg)" });
	...
	tab = tab + "</div><br/><br/><br/><br/><br/></div></div>";
	}
	$JQ("#rasi").html(tab);
